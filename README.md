...
* **Arithmetic expression index** - comparisons combining card fields (power+toughness<4) had no narrowing path and scanned every card. The four fields span only ~564 distinct combinations across 31.5k cards, so the index evaluates the predicate against those and unions the matching postings - exact, 4.15x geometric mean on affected queries, 135KB of index (rust, [pull request](https://github.com/jbylund/sylvan_librarian/pull/750)).
