%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%     Document order     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% The prose lives in sections/; this file is only the order. Reorder these lines to
% reorder the resume.
%
% Order here is editorial, not chronological, which is why the filenames carry no numeric
% prefix: Rice sits after Columbia, Sylvan Librarian before the older pg_mimic, and the
% Sylvan bullets run strongest-first with the parser last despite being written first. A
% numbering scheme would encode a rule this document does not follow.
%
% Each \input ends in %, commenting out the newline that would otherwise follow it, so the
% token stream is byte-identical to the single file this replaced. Verified: same build
% machine, before and after, 1572 words identical in text and position. Dropping the % is
% probably harmless -- the extra newlines land in vertical mode, where they collapse -- but
% identical beats probably.

\input{sections/header}%
\input{sections/experience}%
\input{sections/projects}%
\input{sections/open_source}%
\input{sections/education}%
\input{sections/skills}%

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%    Publications    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\iffalse
\input{publications}
\fi

\begin{center}Last updated \monthname\ \the\year\end{center}
