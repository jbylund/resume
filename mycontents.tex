...
\begin{bullets}
    ...
    \item \blead{In-process query engine}replaced the PostgreSQL search path with an in-memory rust/PyO3 filter engine over 96k cards, {\mytilde}76x faster on a geometric mean of representative queries (0.20ms vs 14.9ms), taking the database out of the hot path (rust, PyO3).
    \item \blead{Query parser}hand-rolled a recursive descent parser for the full query language, {\mytilde}49x faster than the pyparsing grammar it replaced (158k vs 3.2k parses/sec), with a 133 case parity suite asserting both emit identical SQL (python).
\end{bullets}
